...
export default function ResultsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { currentResult, loadResult } = useOsintStore();
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-slate-400 text-sm">
            {r.entity.type === "company" ? <Building2 size={15} /> : <User size={15} />}
            <span className="capitalize">{r.entity.type}</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-white">
            {r.entity.name}
          </h1>
          <p className="text-slate-400 max-w-xl text-sm leading-relaxed">{r.summary}</p>
      ...
      <div className="space-y-10">
        {categories.map((cat) => {
          const adapters = r.results.filter((a) => a.category === cat.id);
          if (adapters.length === 0) return null;
          const totalPoints = adapters.reduce((acc, a) => acc + a.data.length, 0);
          return (
            <section key={cat.id} className="space-y-4">
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-bold text-slate-200">{cat.label}</h2>
                <span className="text-xs font-mono text-slate-500 bg-slate-800 border border-slate-700 px-2 py-0.5 rounded">
                  {totalPoints} findings
                </span>
              </div>
              <div className="space-y-3">
                {adapters.map((adapter) => (
                  <AdapterSection key={adapter.adapterId} adapter={adapter} />
                ))}
              </div>
            </section>
          );
